...
<div id="img">
	<div class="clear"></div>
	        ...
	        if($user_os == "Linux"){
                ...
                $submit_path = str_replace("'","00000", $path);
	        	...
	        	if(isset($word)){
		        	...
		        	if($dir_size != $db_size ){
		        		echo"<input type='button' onclick=\"submit_py('$submit_path')\" value='UPDATE' class='update' >";
		        	}else if(json_encode($img_in) !="[]"){
		        		echo"<input type='button' onclick=\"submit_py('$submit_path')\" value='UPDATE' class='update' >";
		        	} else{}
	        	}
                        ...
                        <li style='width:24%;' id='img' name='cart[]' img_name='$img_rs[img_name]' img_id='$img_rs[img_id]' ondrop=\"vid_drop(event,'$path','$img_rs[img_id]','$img_name','$total_sec')\" ondragover='allowDrop(event)'>
                            <label> 
                               <input type='checkbox' name='images[]' class='checkbox'  value='$img_rs[img_id]'/>
                                ...
                                <br>
                                <img src='images/detail.png' onclick=\"detail('$img_rs[img_id]','$img_name','$img_rs[img_path]','$path','$word')\" style='padding-left:90%'>
                                ");
                                if(isset($word)){
                                    echo "
                                        <center><br>
...
$('.tag_edit').click(function() {   //tag tree -> 태그 수정
	var $text = $(this),
...
function submit_py(path){   //파일 update
	var path = path;
	$.ajax({
		type : "post",
		url : "thum.php",
...
$("#insert").click(function(){  //체크된 파일 카트 담기
...
$("#cart").click(function(){    //cart -> 카트 팝업
	var popUrl = "cart.php";
...
function drop(ev,path,img_id) { //img -> 태그 drop
	ev.preventDefault();
...
function vid_drop(ev,path,img_id,img_name,total_sec) {  //vid -> 태그 drop
    ev.preventDefault();
    var data = ev.dataTransfer.getData("text");
    var path = path;
        // var interval_id = interval_id;
...
function download_interval(img_path,img_name,start_time,end_time){  //detail페이지 -> 구간 다운로드 함수
	$.ajax({
		type : "post",
		url : "video_py.php",
...
function getFrame(){    //현재 프레임 얻는 함수
...
function list(interval_list){   // 태그 클릭 -> 태그에 해당되는 구간들 차례로 재생
...
function now_playing(path){ //현재 재생되는 영상 다운로드
    alert(path);
